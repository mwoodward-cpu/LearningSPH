...
function trapezoid_int_KL(G, G_pred, xs, xf)
	...
	for i in 2 : n_int
		...
		elseif (G_pred(x[i - 1]) == 0.0) || (G_pred(x[i]) == 0.0)
			integrand = 1e3
		# if (abs(G(x[i - 1])) < 1e-6) || (abs(G(x[i])) < 1e-6)
		# 	integrand = 0.0
		# elseif (abs(G_pred(x[i - 1])) < 1e-6) || (abs(G_pred(x[i])) < 1e-6)
		# 	integrand = 1e2
		else
...
function compute_∇L_KL(Vel_inc_gt, Vel_inc_pred, ST)
	...
	for i ∈ 1 : N
		∇L .+= ∂Iklu_∂z[i] .* ST[τ+1, i, 3, :] + ∂Iklv_∂z[i] .* ST[τ+1, i, 4, :]
	end
	return ∇L
end
...
function Ikl_fixed_τ(Vel_inc_gt, Vel_inc_pred, G_u, G_v, hu, hv)
...
function compute_∇L_KL_one(Vel_inc_gt, Vel_inc_pred, ST)
	...
	∂Iklu_∂z = trapezoid_int_∂KLi(G_u, Gu_pred, hu_pred, δu_pr, xus, xue);
	for i ∈ 1 : N
		∇L .+= ∂Iklu_∂z[i] .* ST[τ+1, i, 3, :]
	end
	return ∇L
end


function Ikl_τ(Vel_inc_gt, Vel_inc_pred, G_u, G_v, hu, hv)
    #Forward KL: data is sampled from GT distritubtion
	#For each τ, this integrates over the variable z in kl(τ, z, z_data)
	L = 0.0;
	for τ in 1 : T
...
function Ikl_τ_one(Vel_inc_gt, Vel_inc_pred, G_u, G_v, hu, hv)
    #Forward KL: data is sampled from GT distritubtion
	#For each τ, this integrates over the variable z in kl(τ, z, z_data)
	L = 0.0;
	for τ in 1 : T
...
function compute_∇L_KL_τ(Vel_inc_gt, Vel_inc_pred, ST)
	...
	for τ in 1 : T
		...
		∂Iklv_∂z = trapezoid_int_∂KLi(G_v, Gv_pred, hv_pred, δv_pr, xvs, xve);
		for i ∈ 1 : N
			∇L .+= (∂Iklu_∂z[i] .* ST[τ+1, i, 3, :] + ∂Iklv_∂z[i] .* ST[τ+1, i, 4, :]) * dt
		end
	end
	return ∇L
end
...
function compute_∇L_KL_τ_one(Vel_inc_gt, Vel_inc_pred, ST)
	∇L = zeros(n_params);
	for τ in 1 : T
		δu_gt = Vel_inc_gt[τ, :, 1]; δu_pr = Vel_inc_pred[τ, :, 1];
		Gu_pred(x) = kde(x, δu_pr); hu_pred = obtain_h_kde(δu_pr);
...
function discKL_compute_∇L_t(Vel_inc_gt, Vel_inc_pred, ST)
	∇L = zeros(n_params)
	for τ in 1 : T
		for i ∈ 1 : N
...
function obtain_grid_iterators()
	gsp = 5; #produces grid of 2^gsp x 2^gsp x 2^gsp number of particles
	pgrid = Iterators.product((2*pi/(2^gsp)):(2*pi/(2^gsp)):2*pi, (2*pi/(2^gsp)):(2*pi/(2^gsp)):2*pi)
	pgrid = vec(collect.(pgrid)) #particles
    ...
    for xa_hash in x_hash[1] - 2 : x_hash[1] + 2
      ...
      for ya_hash in x_hash[2] - 2 : x_hash[2] + 2
        ...
        for n2 in hash[xb_hash, yb_hash]
          ...
          if (close)
			for i in 1 : D
	            Vf[n, i] += m * W(sqrt(r2), h) * V[n2, i] / (rho[n2] + 1e-8);
				∂Vf_∂x[n, i] += m * H(sqrt(r2), h) * V[n2, i] * XX[1] / (rho[n2] + 1e-8);
				∂Vf_∂y[n, i] += m * H(sqrt(r2), h) * V[n2, i] * XX[2] / (rho[n2] + 1e-8);
			end
			∂Vf_∂u[n, 1] = m * W(0.0, h) / (rho[n2] + 1e-8);
			∂Vf_∂v[n, 2] = m * W(0.0, h) / (rho[n2] + 1e-8);
          end
        end
    end   end
  end
...
function compute_∇L_field(Vf_gt, Vf_p, ∂Vf_∂x, ∂Vf_∂y, ∂Vf_∂u, ∂Vf_∂v, ST)
	∇L = zeros(n_params)
	for t in 2 : (T + 1)
		for n ∈ 1 : N
...
function compute_L(Vel_inc_gt, Vel_inc_pred, G_u, G_v, hu_kde_gt, hv_kde_gt,
	...
	if loss_method == "kl"
		L = Ikl_fixed_τ(Vel_inc_gt, Vel_inc_pred, G_u, G_v, hu_kde_gt, hv_kde_gt)
	end
	if loss_method =="kl_lf"
		L_kl = Ikl_fixed_τ(Vel_inc_gt, Vel_inc_pred, G_u, G_v, hu_kde_gt, hv_kde_gt)
	...
	if loss_method == "kl_t"
		L = Ikl_τ(Vel_inc_gt, Vel_inc_pred, G_u, G_v, hu_kde_gt, hv_kde_gt)
	end
...
function compute_∇L(loss_method, Vel_inc_gt, Vel_inc_pred, traj_pred, traj_gt, vels_pred, vels_gt, rhos_pred, Vf_gt, ST)
	if loss_method == "l2"
	...
	if loss_method == "kl_lf"
		...
		Vf_p, ∂Vf_∂x, ∂Vf_∂y, ∂Vf_∂u, ∂Vf_∂v =
		obtain_interpolated_velocity_over_τ(traj_pred, vels_pred, rhos_pred, T)
		∇L_lf = compute_∇L_field(Vf_gt, Vf_p, ∂Vf_∂x, ∂Vf_∂y, ∂Vf_∂u, ∂Vf_∂v, ST)
		∇L = ∇L_kl + ∇L_lf
	end
	if loss_method == "kl_t"
	...
	if loss_method =="kl_lf_t"
		...
		∇L_kl = compute_∇L_KL_τ(Vel_inc_gt, Vel_inc_pred, ST)
		∇L_lf = compute_∇L_field(Vf_gt, Vfp, ∂Vf_∂x, ∂Vf_∂y, ∂Vf_∂u, ∂Vf_∂v, ST)
		∇L = ∇L_kl + ∇L_lf
	end
	if loss_method == "dkl_t"
	...
	if loss_method =="lf"
		Vfp, ∂Vf_∂x, ∂Vf_∂y, ∂Vf_∂u, ∂Vf_∂v =
		obtain_interpolated_velocity_over_τ(traj_pred, vels_pred, rhos_pred, T);
		∇L = compute_∇L_field(Vf_gt, Vfp, ∂Vf_∂x, ∂Vf_∂y, ∂Vf_∂u, ∂Vf_∂v, ST)
	end
	return ∇L
end
