...
function dPi_dxi(drhoi, rhoi, ∂P_ρ)
  return ∂P_ρ * drhoi #return ∇Pi = (∂Pᵢ/∂x, ∂Pᵢ/∂y,..)
end

function dPj_dxi(drhoj, rhoj, ∂P_ρ)
  return ∂P_ρ * drhoj #return ∇Pi = (∂Pj/∂xᵢ, ∂Pj/∂yᵢ,..)
end
...
function obtain_sph_diff_A_model_deriv(X, V, p_in)
  ...
  ∂P_ρ = ∂P_∂ρ.(rho)
  for n in 1 : N
    P[n] = Pnn_update(rho[n], p_in)
    # for k in 1 : n_params
    #   ∂P_∂θ_M[n, k] = ∂P_θk(rho[n], k)
    # end
    ∂P_∂θ_M[n, :] .= ∂P_∂θ(rho[n], p_in)

    for i in 1 : D
      dPi[n, i] = dPi_dxi(drhoi_dxi[n, i], rho[n], ∂P_ρ[n]) #can put in loop below to save time?
      dPj[n, i] = dPj_dxi(drhoj_dxi[n, i], rho[n], ∂P_ρ[n])
    end
  end
  for n in 1 : N
    mPdrho2[n] = m * Pnn_d_rho2(rho[n], Pnn_update, p_in);
    for i in 1 : D
        ...
        while (yb_hash > n_hash)    yb_hash -= n_hash;   end
          for n2 in hash[xb_hash, yb_hash]
            ...
            if (close)
              ...
              for k in 1 : n_params
                ∂F_p[n, 3, k]  += -m *(∂P_∂θ_M[n, k]/rho[n]^2 + ∂P_∂θ_M[n2, k]/rho[n2]^2)*XX[1]*H(sqrt(r2), h)
                ∂F_p[n, 4, k]  += -m *(∂P_∂θ_M[n, k]/rho[n]^2 + ∂P_∂θ_M[n2, k]/rho[n2]^2)*XX[2]*H(sqrt(r2), h)
              end
              ...
              #∂ₓF^y:
              ∂F_x[n, 4, 1] += -(XX[2]*XX[1])*(m*Π + mP_rho2)*tmp∂rH_r - m*XX[2]*(∂Π_∂xi + dP_rhx)*tmpH
              #∂yF^y:
