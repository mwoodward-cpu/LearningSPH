      ...
      for ya_hash in x_hash[2] - 2 : x_hash[2] + 2
        ...
        for n2 in hash[xb_hash, yb_hash]
          ...
          if (close)
              ...
              #∂ₓF^x:
              ∂F_x[n, 3, 1] += (∂fij_x1 * ∂Prhoi_∂x + ∂fij_x2 * ∂Prhoj_∂x +
                                ∂fij_x3 * VV[1] + ∂fij_x4 * XX[1]/(sqrt(r2) + 1e-4))*XX[1] +
                                rot_term;
              #∂yF^x:
              ∂F_x[n, 3, 2] += (∂fij_x1 * ∂Prhoi_∂y + ∂fij_x2 * ∂Prhoj_∂y +
                                ∂fij_x3 * VV[2] + ∂fij_x4 * XX[2]/(sqrt(r2) + 1e-4))*XX[1];

              #∂uF^x:
              ∂F_x[n, 3, 3] += (∂fij_x3 * XX[1])*XX[1]
                                #+θ/tke *(1 - 1/N) - θ/N*V[n, 1]/(tke^2)*(V[n, 1] - mean(V[:, 1]));
              #∂vF^x:
              ∂F_x[n, 3, 4] += (∂fij_x3 * XX[2])*XX[1];

              #∂ₓF^y:
              ∂F_x[n, 4, 1] += (∂fij_x1 * ∂Prhoi_∂x + ∂fij_x2 * ∂Prhoj_∂x +
                                ∂fij_x3 * VV[1] + ∂fij_x4 * XX[1]/(sqrt(r2) + 1e-4))*XX[2]
              #∂yF^y:
              ∂F_x[n, 4, 2] += (∂fij_x1 * ∂Prhoi_∂y + ∂fij_x2 * ∂Prhoj_∂y +
                                ∂fij_x3 * VV[2] + ∂fij_x4 * XX[2]/(sqrt(r2) + 1e-4))*XX[2] +
                                rot_term;
              #∂uF^y:
              ∂F_x[n, 4, 3] += (∂fij_x3 * XX[1])*XX[2];
              #∂vF^y:
              ∂F_x[n, 4, 4] += (∂fij_x3 * XX[2])*XX[2]
                                #+ θ/tke *(1 - 1/N) - θ/N*V[n, 2]/(tke^2)*(V[n, 2] - mean(V[:, 2]));
            end
          end
    end   end
  end
