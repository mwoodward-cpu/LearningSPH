...
const vis_rate = 5;		#sampling frequency for output
const lr = 4e-2; 			#initial lr (later adapted with ADAM)
...
method = "phys_inf_Wab_theta_po_liv_Pi"
...
if method=="phys_inf_Wab_theta_po_liv_Pi"
	...
	a_hat = 2.932; b_hat = 3.559; po_hat = 0.0449; θ_hat = θ;
	p_hat = [c_hat, α_hat, β_hat, g_hat, a_hat, b_hat, po_hat, θ_hat];
...
function training_algorithm(l_method, n_itrs, vis_rate, T, p_h)
	...
	rot_RF = zeros(range_itrs)
	gal_inv = zeros(range_itrs)
	...
	for k ∈ 1 : n_itrs
        ...
        if mod(k, vis_rate) == 0
			...
			if (method == "phys_inf_Wab_theta_po_liv_Pi")
				if p_h[5] <=2.0 #to avoid undefined gamma function in W_ab at a<2
						p_h[5] = 2.0 + 1e-9
				end
				c_itr[ii] = p_h[1]; α_itr[ii] = p_h[2];
				β_itr[ii] = p_h[3]; g_itr[ii] = p_h[4];
				a_itr[ii] = p_h[5]; b_itr[ii] = p_h[6];
				po_itr[ii] = p_h[7]; θ_itr[ii] = p_h[8];
				println("Itr  = ", k, "  c_hat = ", p_h[1],  "  α_hat = ", p_h[2], "  β_hat = ", p_h[3],
								"  g_hat = ", p_h[4], "  a_hat = ", p_h[5], "  b_hat = ", p_h[6], "  po_hat = ", p_h[7], "  θ_hat = ", p_h[8], "   $(loss_method): Loss = ", L)
			end

			rot_Q, rot_RF[ii] = rotational_metric(p_h, obtain_sph_AV_A)
			gal_Y_shift, gal_inv[ii] = translational_metric(p_h, obtain_sph_AV_A)
    ...
    return p_h, Vel_inc_pred_k, Vfp_k, L_itr, rot_RF, gal_inv, c_itr, α_itr, β_itr, g_itr, a_itr, b_itr, po_itr, θ_itr
end

p_fin, Vel_inc_pred_k, Vfp_k, L_out, rot_RF, galilean_inv, c_out, α_out, β_out, g_out, a_out, b_out, po_out, θ_out =
...
save_output_data(a_out, "./$(data_out_path)/a_out.npy")
save_output_data(b_out, "./$(data_out_path)/b_out.npy")
