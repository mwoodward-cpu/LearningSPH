        ...
        for za_hash in x_hash[3] - 2 : x_hash[3] + 2
          ...
          for n2 in hash[xb_hash, yb_hash, zb_hash]
            ...
            if (close)
              ...
              ∂F_p[n, 4, 1:(end-1)] .+= - m * ∂f_∂θk(feature_ij, pnn_in) * tmpH * XX[1];
              ∂F_p[n, 5, 1:(end-1)] .+= - m * ∂f_∂θk(feature_ij, pnn_in) * tmpH * XX[2];
              ∂F_p[n, 6, 1:(end-1)] .+= - m * ∂f_∂θk(feature_ij, pnn_in) * tmpH * XX[3];
