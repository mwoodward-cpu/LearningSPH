...
if restart=="true"
  params_path = "./output_data_unif_tracers_forward_node_norm_lf_itr701_lr0.003_T30_θ0.00430899795067121_h0.335_tcoarse2_dt0.04_height6_mnode_norm_llf_klswitch1/params_fin.npy"
...
function obtain_sph_diff_A_model_deriv(X, V, p_in)
  ...
  for n in 1 : N
    ...
    #∂uF^x:
    ∂F_x[n, 4, 4] = sum(∂f_x[3*n_list + 1 : 4*n_list]) + θ / (2*tke)
    #∂vF^x:
    ...
    #∂vF^y:
    ∂F_x[n, 5, 5] = sum(∂f_y[4*n_list + 1 : 5*n_list]) + θ / (2*tke)
    #∂wF^y:
    ...
    #∂wF^z:
    ∂F_x[n, 6, 6] = sum(∂f_z[5*n_list + 1 : 6*n_list]) + θ / (2*tke);
