...
function fij_ext(Vn, meanV, tke, θ, i)
  f_ext = (θ / tke) * (Vn[i] .- meanV[i])
  return f_ext
end

function ∂fij_p(Vn, meanV, tke, θ, i)
    return ForwardDiff.derivative(θ -> fij_ext(Vn, meanV, tke, θ, i), θ)
end
...
function obtain_sph_diff_A_model_deriv(X, V, p_in)
        ...
        for za_hash in x_hash[3] - 2 : x_hash[3] + 2
          ...
          for n2 in hash[xb_hash, yb_hash, zb_hash]
            ...
            if (close)
              ...
              ∂F_p[n, 4, end] = ∂fij_p(V[n,:], meanV, tke, θ_in, 1)
              ∂F_p[n, 5, 1:(end-1)] .+= ∂fy_∂θk(feature_ij, pnn_in)
              ∂F_p[n, 5, end] = ∂fij_p(V[n,:], meanV, tke, θ_in, 2)
              ∂F_p[n, 6, 1:(end-1)] .+= ∂fz_∂θk(feature_ij, pnn_in)
              ∂F_p[n, 6, end] = ∂fij_p(V[n,:], meanV, tke, θ_in, 3)
  ...
  # println("mean rho = ", mean(rho))
  #tke = 0.5*mean(rho .* (V[: ,1].^2 .+ V[: ,2].^2 .+ V[: ,3].^2));
  for i in 1 : D
    A[:, i] += (θ_in/(2*tke)) * (V[:, i] .- mean(V[:, i]))
  end
