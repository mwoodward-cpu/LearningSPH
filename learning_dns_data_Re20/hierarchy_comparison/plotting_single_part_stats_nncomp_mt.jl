...
function load_all_vel_inc(m_all, T)
	...
	ii = 1;
	for m in m_all
		vel_inc_m[:,:,:,ii] = load_vel_inc(m, Mt)
		ii += 1;
	end
	return vel_inc_m
end
...
function comparing_GV_log(width=1.2)
    ...
    display(plt)
    out_path = "./learned_figures/Gv_trained_comp_log_$(m_kind)_t$(T)_mt$(Mt).png"
...
function comparing_GV(width=1.2)
    ...
    display(plt)
    out_path = "./learned_figures/Gv_trained_comp_$(m_kind)_t$(T)_mt$(Mt).png"
...
function comparing_Gd_log(width=1.2)
    ...
    display(plt)
    out_path = "./learned_figures/Gd_trained_comp_log_$(m_kind)_t$(T)_mt$(Mt).png"
...
function comparing_Gd(width=1.2)
    ...
    display(plt)
    out_path = "./learned_figures/Gd_trained_comp_$(m_kind)_t$(T)_mt$(Mt).png"
...
function animate_dist_log(width=0.21, sim_time = 30)
	sim_path = "./learned_sims/animate_dist_accl_log_$(m_kind)_t$(T)_mt$(Mt).mp4"
	x_s = -width; x_e = width;
...
function animate_dist(width=0.21, sim_time = 30)
	sim_path = "./learned_sims/animate_dist_accl_$(m_kind)_t$(T)_$(Mt).mp4"
	x_s = -width; x_e = width;
	anim = @animate for t ∈ 25:T
		println("time step = ", t); t_s = round(t*dt, digits=1)
		plt = plot(x->Gat_m(x, t, 1), x_s, x_e, label=m_all_l[1],
