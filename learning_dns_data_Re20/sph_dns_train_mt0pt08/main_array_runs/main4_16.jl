...
method = "phys_inf_theta_liv_Pi_fixcg"
...
α_hat = α; β_hat = β; θ_hat = θ;
p_hat = [α_hat, β_hat, θ_hat];
...
function training_algorithm(l_method, n_itrs, vis_rate, T, p_h)
	...
	for k ∈ 1 : n_itrs
        ...
        if mod(k, vis_rate) == 0
			...
			L_itr[ii] = L;
			α_itr[ii] = p_h[1]; β_itr[ii] = p_h[2]; θ_itr[ii] = p_h[3];
			println("Itr  = ", k, "  α_hat = ", p_h[1],  "  β_hat = ", p_h[2], "  θ_hat = ", p_h[3],
			   	     "   $(loss_method): Loss = ", L)
