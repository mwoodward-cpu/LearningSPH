...
function obtain_pred_dists(traj_pred, vels_pred, r0, v0)
    ...
    for m in 1 : N
        for τ in 1 : num_tau
            for i in 1 : D
                Diff_pred_vec[τ, m, i] = traj_pred[1 + τ, m, i] - r0[m, i] - dt * τ * v0[m, i]
                Vel_inc_pred[τ, m, i] = vels_pred[1 + τ, m, i] - v0[m, i];
                while (Diff_pred_vec[τ, m, i] > pi)   Diff_pred_vec[τ, m, i] -= 2. * pi;   end
                while (Diff_pred_vec[τ, m, i] < -pi)   Diff_pred_vec[τ, m, i] += 2. * pi;   end
            end
            Diff_pred[τ, m] = sqrt(sum((Diff_pred_vec[τ, m, :]).^2))
        end
    end
    return Diff_pred, Vel_inc_pred
end

function obtain_gt_dists(traj_gt, vels_gt, r0, v0)
    Diff_vec = zeros(num_tau, N_f, D)
    Diff = zeros(num_tau, N_f)
    Vel_inc = zeros(num_tau, N_f, D)
    for m in 1 : N_f
        for τ in 1 : num_tau
            for i in 1 : D
                Diff_vec[τ, m, i] = traj_gt[1 + τ, m, i] - r0[m, i] - dt * τ * v0[m, i]
                Vel_inc[τ, m, i] = vels_gt[1 + τ, m, i] - v0[m, i];
                while (Diff_vec[τ, m, i] > pi)   Diff_vec[τ, m, i] -= 2. * pi;   end
                while (Diff_vec[τ, m, i] < -pi)   Diff_vec[τ, m, i] += 2. * pi;   end
            end
            Diff[τ, m] = sqrt(sum((Diff_vec[τ, m, :]).^2))
...
Diff_gt, Vel_inc_gt = obtain_gt_dists(traj_gt, vels_gt, traj_gt[1,:,:], vels_gt[1,:,:])
