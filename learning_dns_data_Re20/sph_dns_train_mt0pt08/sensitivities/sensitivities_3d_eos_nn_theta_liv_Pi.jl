...
function compute_Π(Pnn, XX, VV, rhoi, rhoj, α, β, h)
  ci = compute_ca(Pnn, rhoi);
  cj = compute_ca(Pnn, rhoj);
  c_bar = (ci + cj)/2;
  if (XX'*VV < 0)
...
# function compute_ca(rho, Pnn, p_in)
#   dp_dρ = ForwardDiff.derivative(x -> Pnn(x, p_in), rho)
#   return sqrt(dp_dρ)
#   # return sqrt(g) * c * rho^(0.5*(g - 1))
# end

function compute_ca(Pnn, rho)
        dp_dρ = x -> ForwardDiff.derivative(Pnn, x)
        c_ = sqrt(abs(dp_dρ(rho)))
        return c_
end
...
#---------------∂Π/∂x
function ∂Π_x(Pnn, XX, VV, h, α, β, rhoi, rhoj, ∂ρi_xi, ∂ρj_xi)
  ci = compute_ca(Pnn, rhoi);
  cj = compute_ca(Pnn, rhoj);
  c_bar = (ci + cj)/2;
  if (XX'*VV < 0)
...
function ∂Π_y(Pnn, XX, VV, h, α, β, rhoi, rhoj, ∂ρi_yi, ∂ρj_yi)
  ci = compute_ca(Pnn, rhoi);
  cj = compute_ca(Pnn, rhoj);
  c_bar = (ci + cj)/2;
  if (XX'*VV < 0)
...
function ∂Π_z(Pnn, XX, VV, h, α, β, rhoi, rhoj, ∂ρi_zi, ∂ρj_zi)
  ci = compute_ca(Pnn, rhoi);
  cj = compute_ca(Pnn, rhoj);
  c_bar = (ci + cj)/2;
  if (XX'*VV < 0)
...
function ∂Π_u(Pnn, XX, VV, h, α, β, rhoi, rhoj)
  ci = compute_ca(Pnn, rhoi);
  cj = compute_ca(Pnn, rhoj);
  c_bar = (ci + cj)/2;
  if (XX'*VV < 0)
...
function ∂Π_v(Pnn, XX, VV, h, α, β, rhoi, rhoj)
  ci = compute_ca(Pnn, rhoi);
  cj = compute_ca(Pnn, rhoj);
  c_bar = (ci + cj)/2;
  if (XX'*VV < 0)
...
function ∂Π_w(Pnn, XX, VV, h, α, β, rhoi, rhoj)
  ci = compute_ca(Pnn, rhoi);
  cj = compute_ca(Pnn, rhoj);
  c_bar = (ci + cj)/2;
  if (XX'*VV < 0)
      ...
      for ya_hash in x_hash[2] - 2 : x_hash[2] + 2
        ...
        for za_hash in x_hash[3] - 2 : x_hash[3] + 2
          ...
          for n2 in hash[xb_hash, yb_hash, zb_hash]
            ...
            if (close)
              Π = compute_Π(Pnn, XX, VV, rho[n], rho[n2], α, β, h)
              ...
              ∂Π_∂xi = ∂Π_x(Pnn, XX, VV, h, α, β, rho[n], rho[n2], drhoi_dxi[n, 1], drhoj_dxi[n, 1])
              ∂Π_∂yi = ∂Π_y(Pnn, XX, VV, h, α, β, rho[n], rho[n2], drhoi_dxi[n, 2], drhoj_dxi[n, 2])
              ∂Π_∂zi = ∂Π_z(Pnn, XX, VV, h, α, β, rho[n], rho[n2], drhoi_dxi[n, 3], drhoj_dxi[n, 3])
              ∂Π_∂ui = ∂Π_u(Pnn, XX, VV, h, α, β, rho[n], rho[n2])
              ∂Π_∂vi = ∂Π_v(Pnn, XX, VV, h, α, β, rho[n], rho[n2])
              ∂Π_∂wi = ∂Π_w(Pnn, XX, VV, h, α, β, rho[n], rho[n2])
