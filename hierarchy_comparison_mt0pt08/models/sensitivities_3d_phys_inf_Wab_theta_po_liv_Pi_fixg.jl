...
function H(r,h,a,b) # H(r) = (d W / d r) / r
  sigma = sigma_f(h,a,b);
  q = r / (2*h);
  if (q > 1.0)
    return 0.0
...
function dH_dr_r(r, h, a, b)
  ...
  return sigma * a*b/(r*8*h^3) * q^(a-3) * (1 - q^a)^(b-2) * ((a*b - 2)*q^a - a + 2)
end
...
function fij(n, n2, rho, XX, VV, Vn, meanV, tke, r2, p, h, m, i)
  c_in, α_in, β_in, a_in, b_in, po_in, θ_in = p
  Pi = Pres(rho[n], c_in, g, po_in)
  Pj = Pres(rho[n2], c_in, g, po_in)
  Π = compute_Π(XX, VV, rho[n], rho[n2], α_in, β_in, h, c_in, g)
  summand = - m * (Pi/(rho[n]^2) + Pj/(rho[n2]^2) + Π) * XX[i] * H(sqrt(r2), h, a_in, b_in)
...
function obtain_sph_diff_A_model_deriv(X, V, p_in)
  c_in, α_in, β_in, a_in, b_in, po_in, θ_in = p_in
      ...
      for ya_hash in x_hash[2] - 2 : x_hash[2] + 2
        ...
        for za_hash in x_hash[3] - 2 : x_hash[3] + 2
          ...
          for n2 in hash[xb_hash, yb_hash, zb_hash]
            ...
            if (close)
              tmp = m * W(sqrt(r2), h, a_in, b_in); rho[n] += tmp;
              for i in 1 : D
                drhoi_dxi[n, i] += m * H(sqrt(r2), h, a_in, b_in) * XX[i] #computes ∂ρᵢ/∂xᵢ
                drhoj_dxi[n, i] = m * H(sqrt(r2), h, a_in, b_in) * XX[i] #computes ∂ρⱼ/∂xᵢ
              end
            end
          end
        end
  ...
  for n in 1 : N
    mPdrho2[n] = m * P_d_rho2(rho[n], Pres, c_in, g, po_in);
  end
      ...
      for ya_hash in x_hash[2] - 2 : x_hash[2] + 2
        ...
        for za_hash in x_hash[3] - 2 : x_hash[3] + 2
          ...
          for n2 in hash[xb_hash, yb_hash, zb_hash]
            ...
            if (close)
              Π = compute_Π(XX, VV, rho[n], rho[n2], α_in, β_in, h, c_in, g)
              tmp = -(mPdrho2[n] + mPdrho2[n2] + m*Π) * H(sqrt(r2), h, a_in, b_in);
              for i in 1 : D
              ...
              tmpH = H(sqrt(r2), h, a_in, b_in);
              tmpW = W(sqrt(r2), h, a_in, b_in);
              ...
              tmp∂rH_r = dH_dr_r(sqrt(r2), h, a_in, b_in);
              # println("r2 = ", r2)
              if r2 == 0.0
                tmp∂rH_r = 1e6
              end
              ...
              dP_rhx = (∂P_rhoi2_∂x(rho[n], drhoi_dxi[n, 1], c_in, g, po_in) + ∂P_rhoi2_∂x(rho[n2], drhoj_dxi[n2, 1], c_in, g, po_in))
              dP_rhy = (∂P_rhoi2_∂y(rho[n], drhoi_dxi[n, 2], c_in, g, po_in) + ∂P_rhoi2_∂y(rho[n2], drhoj_dxi[n2, 2], c_in, g, po_in))
              dP_rhz = (∂P_rhoi2_∂z(rho[n], drhoi_dxi[n, 3], c_in, g, po_in) + ∂P_rhoi2_∂z(rho[n2], drhoj_dxi[n2, 3], c_in, g, po_in))

              ∂Π_∂xi = ∂Π_x(XX, VV, c_in, g, h, α_in, β_in, rho[n], rho[n2], drhoi_dxi[n, 1], drhoj_dxi[n, 1])
              ∂Π_∂yi = ∂Π_y(XX, VV, c_in, g, h, α_in, β_in, rho[n], rho[n2], drhoi_dxi[n, 2], drhoj_dxi[n, 2])
              ∂Π_∂zi = ∂Π_z(XX, VV, c_in, g, h, α_in, β_in, rho[n], rho[n2], drhoi_dxi[n, 3], drhoj_dxi[n, 3])
              ∂Π_∂ui = ∂Π_u(XX, VV, c_in, g, h, α_in, β_in, rho[n], rho[n2])
              ∂Π_∂vi = ∂Π_v(XX, VV, c_in, g, h, α_in, β_in, rho[n], rho[n2])
              ∂Π_∂wi = ∂Π_w(XX, VV, c_in, g, h, α_in, β_in, rho[n], rho[n2])
