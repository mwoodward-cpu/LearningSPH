...
function coarse_grain_time(traj, vels, rhos, t_coarse, N, D)
	t_gt, N, D = size(traj)
	t_range = floor(Int, t_gt/t_coarse)
	traj_out = zeros(t_range, N, D);
	vels_out = zeros(t_range, N, D);
	rhos_out = zeros(t_range, N);
	for t in 1 : t_range
		traj_out[t,:,:] = traj[t*t_coarse,:,:]
		vels_out[t,:,:] = vels[t*t_coarse,:,:]
		rhos_out[t,:] = rhos[t*t_coarse,:]
	end
	return traj_out, vels_out, rhos_out
end
