...
"""
				Senstivity analysis Phys_inf wendlend kernel
...
function W(r, h)
  ...
  if q > 2.0
    ...
    out = 21/(16*pi*h^3) * (1 - q/2)^4 * (2*q +1);
  end
  return out
end
...
function H(r, h)
  ...
  if q > 2.0
    ...
    if r > 1e-12
        out = -5/h * 21/(16*pi*h^3) * q * (1 - q/2)^3 / r;
    else
        out = 0.0;
    end
  end
...
function dH_dr_r(r, h)
  out = ForwardDiff.derivative(x-> H(x,h), r)/(r+1e-12)
  return out
end
