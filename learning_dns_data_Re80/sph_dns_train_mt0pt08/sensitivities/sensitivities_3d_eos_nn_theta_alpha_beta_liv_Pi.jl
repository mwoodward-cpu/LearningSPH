...
The parameters within artificial viscosity are also learned
...
function fij_ext(Vn, XX, VV, Pi, Pj, rhoi, rhoj, r2, tke, p, i)
  α_in, β_in, θ_in = p;
...
function ∂fij_p(Vn, XX, VV, Pi, Pj, rhoi, rhoj, r2, tke, p, i)
    return ForwardDiff.gradient(p -> fij_ext(Vn, XX, VV, Pi, Pj, rhoi, rhoj, r2, tke, p, i), p)
end
...
function obtain_sph_diff_A_model_deriv(X, V, p_in)
  ...
  for n in 1 : N
    ...
    ∂P_∂θ_M[n, 1:(end-3)] = ∂P_∂θ(rho[n], pnn_in)
    for i in 1 : D
        ...
        for za_hash in x_hash[3] - 2 : x_hash[3] + 2
          ...
          for n2 in hash[xb_hash, yb_hash, zb_hash]
            ...
            if (close)
              ...
              ∂F_p[n, 4, (end-2):end] .= ∂fij_p(V[n,:], XX, VV, P[n], P[n2], rho[n], rho[n2], r2, tke, p_in[(end-2):end], 1)
              ∂F_p[n, 5, (end-2):end] .= ∂fij_p(V[n,:], XX, VV, P[n], P[n2], rho[n], rho[n2], r2, tke, p_in[(end-2):end], 2)
              ∂F_p[n, 6, (end-2):end] .= ∂fij_p(V[n,:], XX, VV, P[n], P[n2], rho[n], rho[n2], r2, tke, p_in[(end-2):end], 3)
