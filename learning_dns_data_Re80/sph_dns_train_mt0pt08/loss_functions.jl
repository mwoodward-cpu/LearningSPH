...
function Ikl_fixed_τ(Vel_inc_gt, Vel_inc_pred, G_u, G_v, G_w, hu, hv, hw)
...
function Ikl_τ(Vel_inc_gt, Vel_inc_pred)
    #Forward KL: data is sampled from GT distritubtion
	#For each τ, this integrates over the variable z in kl(τ, z, z_data)
	L = 0.0;
	for τ in 2: 2: T
		...
		Gu(x) = Gt_u(x, τ); Gv(x) = Gt_v(x, τ); Gw(x) = Gt_w(x, τ);
                ...
                ηu = hut_kde_gt(τ); ηv = hvt_kde_gt(τ); ηw = hwt_kde_gt(τ)
...
function compute_∇L_KL_τ(Vel_inc_gt, Vel_inc_pred, ST)
	...
	for τ in 2: 2: T
		...
		∂Iklu_∂z = trapezoid_int_∂KLi(Gu, Gu_pred, hu_pred, δu_pr, xus, xue);
		∂Iklv_∂z = trapezoid_int_∂KLi(Gv, Gv_pred, hv_pred, δv_pr, xvs, xve);
		∂Iklw_∂z = trapezoid_int_∂KLi(Gw, Gw_pred, hw_pred, δw_pr, xws, xwe);
		for i ∈ 1 : N
			∇L .+= dt * (∂Iklu_∂z[i] .* ST[τ+1, i, 4, :] + ∂Iklv_∂z[i] .* ST[τ+1, i, 5, :] + ∂Iklw_∂z[i] .* ST[τ+1, i, 6, :])
			# ∇L .+= ∂Iklu_∂z[i] .* ST[τ+1, i, 4, :] + ∂Iklv_∂z[i] .* ST[τ+1, i, 5, :] + ∂Iklw_∂z[i] .* ST[τ+1, i, 6, :]
		end
	end
	return ∇L
...
function display_grid(pos)
	n_2 = round(Int, N/2);
	plt = Plots.scatter(pos[1:n_2, 1], pos[1:n_2, 2], pos[1:n_2, 3],
	title = L"\textrm{Grid}", xlims = [0, 2*pi], ylims = [0,2*pi], zlims = [0,2*pi], legend = false)
	Plots.scatter!(pos[(n_2+1):end, 1], pos[(n_2+1):end, 2], pos[(n_2+1):end, 3], color = "red")
	display(plt)
end

# X_grid = obtain_uniform_mesh(D, gsp)
...
function obtain_interpolated_velocity_over_τ(X_grid, traj, vels, rhos, T, N_f)
	Vf_t = zeros(T+1,N,D);
	...
	for t in 1 : (T+1)
		...
		obtain_interpolated_velocity(X_grid, traj[t,:,:], vels[t,:,:], rhos[t,:], N_f)
	end
...
# X_grid_32 = obtain_uniform_mesh(D, 5)
# Vf_gt_32,d1,d2,d3,d4,d5,d6 = obtain_interpolated_velocity_over_τ(X_grid_32, traj_gt, vels_gt, rhos_gt, T, (2^5)^3)
...
function compute_L(Vel_inc_gt, Vel_inc_pred, G_u, G_v, hu_kde_gt, hv_kde_gt,
	...
	if loss_method == "kl_lf_t"
		Lkl = Ikl_τ(Vel_inc_gt, Vel_inc_pred)
		Lf = mse(Vfp, Vf_gt);
		L = Lkl + Lf;
	end
