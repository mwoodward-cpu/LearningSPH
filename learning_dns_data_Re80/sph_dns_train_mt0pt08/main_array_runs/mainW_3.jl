...
const t_coarse = parse(Int, ARGS[7]);
...
println(" theta = ", θ)
	...
	a_hat = 2.932; b_hat = 3.559; po_hat = -0.25; θ_hat = θ;
...
#	Vfp_k = zeros(range_itrs, T+1, N, D);
	...
	for k ∈ 1 : n_itrs
		...
		update!(opt, p_h, ∇L)
                #fixing other parameters and only learning smoothing kernel:
                p_h[1] = c; p_h[2] = α; p_h[3] = β; p_h[4] = g; p_h[7] = po_hat; p_h[8] = θ;
    end
    return p_h, L_itr, c_itr, α_itr, β_itr, g_itr, a_itr, b_itr, po_itr, θ_itr
end

p_fin, L_out, c_out, α_out, β_out, g_out, a_out, b_out, po_out, θ_out =
