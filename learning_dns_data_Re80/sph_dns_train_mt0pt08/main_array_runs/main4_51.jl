...
const lr = 5e-3; 			#initial lr (later adapted with ADAM)
...
#const height = 8;                                       #height of NN (see sensitivities of NN structure)
...
function training_algorithm(l_method, n_itrs, vis_rate, T, p_h)
	...
	for k ∈ 1 : n_itrs
        ...
        if mod(k, vis_rate) == 0
			...
			θ_itr[ii] = p_h[end];
		    println("Itr  = ", k, "  θ_hat = ", p_h[end], "   $(loss_method): Loss = ", L)
