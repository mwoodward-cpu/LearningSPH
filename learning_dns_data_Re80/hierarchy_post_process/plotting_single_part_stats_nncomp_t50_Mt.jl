...
function comparing_GV_log(width=1.2)
    ...
    display(plt)
    out_path = "./learned_figures_t50/Gv_trained_comp_log_$(m_kind)_t$(T)_Mt$(Mt).png"
...
function comparing_GV(width=1.2)
	...
	plot!(x->G_gt(x), x_s, x_e, label=L"G_{truth}",
		  linewidth = 2.5, legendfontsize=12, color="indigo")


	title!(L"\textrm{Learned - Distributions: }  M_t = %$Mt", titlefont=22)
    # title!(L"\textrm{Comparing } G(\tau,z) \textrm{ and } \hat{G}_{\theta}(\tau,z)", titlefont=20)
    ...
    display(plt)
    out_path = "./learned_figures_t50/Gv_trained_comp_$(m_kind)_t$(T)_Mt$(Mt).png"
...
function comparing_Gd_log(width=1.2)
	...
	plot!(x->Gd_gt(x), x_s, x_e, label=L"G_{truth}",
		  linewidth = 2.5, yaxis=:log, legendfontsize=12, color="indigo")
    ...
    display(plt)
    out_path = "./learned_figures_t50/Gd_trained_comp_log_$(m_kind)_t$(T)_Mt$(Mt).png"
...
function comparing_Gd(width=1.2)
    ...
    display(plt)
    out_path = "./learned_figures_t50/Gd_trained_comp_$(m_kind)_t$(T)_Mt$(Mt).png"
...
function comparing_Ga_log(width=1.2)
    ...
    plot!(x->Ga_m(x, 4), x_s, x_e, label=m_all_l[4], markershape=:diamond, ms = 5.5,
	  	  linestyle=:dashdotdot, linewidth = 2.5, yaxis=:log, legendfontsize=16, color="turquoise")
if m_kind == "nn_comp"
	plot!(x->Ga_m(x, 5), x_s, x_e, label=m_all_l[5], markershape=:rect, ms = 5.5,
	  	  linestyle=:dash, linewidth = 2.5, yaxis=:log, legendfontsize=16, color="gold")
	  end
	...
	title!(L"\textrm{Learned Acceleration: }  M_t = %$Mt", titlefont=20)
	      # title!(L"\textrm{Comparing } G(\tau,z) \textrm{ and } \hat{G}_{\theta}(\tau,z)", titlefont=20)
	xlabel!(L"\textrm{Acceleration} ", xtickfontsize=14, xguidefontsize=20)
	ylabel!(L"log(G_a)", ytickfontsize=14, yguidefontsize=20)

    display(plt)
    out_path = "./learned_figures_t50/Ga_trained_comp_log_$(m_kind)_t$(T)_Mt$(Mt).png"
...
if m_kind == "nn_comp"
    ...
    display(plt)
    out_path = "./learned_figures_t50/Ga_trained_comp_$(m_kind)_t$(T)_Mt$(Mt).png"
...
function animate_dist(width=0.21, sim_time = 30)
	sim_path = "./learned_sims_t50/animate_dist_accl_$(m_kind)_t$(T)_Mt$(Mt).mp4"
	x_s = -width; x_e = width;
	anim = @animate for t ∈ 25:T
		println("time step = ", t); t_s = round(t*dt, digits=1)
		plt = plot(x->Gat_m(x, t, 1), x_s, x_e, label=m_all_l[1],
...
function obtain_ga_over_all_t(width=0.1)
	...
	plot!(x->Gat2_gt(x), x_s, x_e, label=L"G_{truth}",
		  ...
		  title!(L"\textrm{Learned Acceleration } M_t = %$Mt", titlefont=26)
	      xlabel!(L"\textrm{Acceleration} ", xtickfontsize=18, xguidefontsize=24)
	      ylabel!(L"G_a", ytickfontsize=18, yguidefontsize=24)
    display(plt)
	out_path = "./learned_figures_t50/Ga_comb_t_comp_$(m_kind)_t$(T)_Mt$(Mt).png"

	savefig(plt, out_path)
end
obtain_ga_over_all_t(0.04)
...
if m_kind == "nn_comp"
    ...
    display(plt)
	out_path = "./learned_figures_t50/Ga_log_comb_t_comp_$(m_kind)_t$(T)_Mt$(Mt).png"

	savefig(plt, out_path)
end
# obtain_ga_log_over_all_t(0.04)
