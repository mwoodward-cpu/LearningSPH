...
function comparing_G_all(G, Gin_gt, yaxis_log, stat_obj, width=1.2)
	...
	title!(L"\textrm{Learned ~ Distributions: ~ Over ~ time } \sim t_{eddy}", titlefont=title_fs)
	if stat_obj=="dv"
...
gv_w = T * 0.0012
comparing_G_all(G_m, G_gt, 0, "dv", gv_w)
#
gd_w = T^(1.35) * 0.0014;
...
function animate_dist(width=0.21, sim_time = 30)
	sim_path = "./learned_sims/animate_dist_accl_$(m_kind)_t$(T)_Mt$(Mt).mp4"
	x_s = -width; x_e = width;
	anim = @animate for t ∈ 25:T
		println("time step = ", t); t_s = round(t*dt, digits=1)
		plt = plot(x->Gat_m(x, t, 1), x_s, x_e, label=m_all_l[1],
