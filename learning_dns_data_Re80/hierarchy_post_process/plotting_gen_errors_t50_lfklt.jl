...
include("utils.jl")
make_dir("learned_figures_t50_lfklt")
...
function load_losses(method, T_pred, T_train)
        # Lf = npzread("./learned_generalization_t50_lfklt/lf_loss_t_$(method)_Tp$(T_pred)_Tt$(T_train).npy")
        # Lkl_lf = npzread("./learned_generalization_t50_lfklt/kl_lf_loss_t_$(method)_Tp$(T_pred)_Tt$(T_train).npy")
        Lkl_t = npzread("./learned_generalization_t50_lfklt/kl_t_loss_t_$(method)_Tp$(T_pred)_Tt$(T_train)_over_t.npy")
        Lf_norm = npzread("./learned_generalization_t50_lfklt/lf_norm_loss_t_$(method)_Tp$(T_pred)_Tt$(T_train)_over_t.npy")
        return Lkl_t[1:n_loss], Lf_norm[1:n_loss]
end


function obtain_box_plot_t_lf_comb(methods, Lf_norm)
    gr(size=(900,750))

    path_lf = "./learned_figures_t50_lfklt/L_lf_box_plot_over_t_T$(T)_$(m_runs)_$(l_method).png"
    path_lf_norm = "./learned_figures_t50_lfklt/L_lf_norm_box_plot_over_t_T$(T)_$(m_runs)_$(l_method).png"
    ...
    title!(L"\textrm{Generalization ~ Error ~ Over: ~ t} ", titlefont=title_fs)
...
function obtain_box_plot_t_lf_phys(methods, Lf_norm)
    gr(size=(900,750))
...
function obtain_all_losses(all_methods, T_pred, T_train)
    ...
    for m_ in all_methods
        Lkl_t[:, ii], Lf_norm[:, ii] = load_losses(m_, T_pred, T_train)
		# if m_ == "rot_inv_theta_liv_Pi"
		# 	Lf_norm_ = npzread("./learned_generalization_t50_lfklt/lf_norm_loss_t_$(m_)_Tp500_Tt50_over_t.npy")
		# 	Lf_norm[:, ii] = Lf_norm_[1:n_loss]
		# end
        ii += 1;
    end
...
if m_runs=="phys_inf"
	...
	obtain_box_plot_t_lf_phys(methods_phys, Lf_norm)
end
...
if m_runs=="comb"
	...
	obtain_box_plot_t_lf_comb(methods_comb, Lf_norm)
end
