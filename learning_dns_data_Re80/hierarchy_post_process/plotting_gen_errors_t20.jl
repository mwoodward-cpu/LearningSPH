...
function load_losses(method, T_pred, T_train)
        Lf = npzread("./learned_generalization/lf_loss_t_$(method)_Tp$(T_pred)_Tt$(T_train).npy")
        Lkl_lf = npzread("./learned_generalization/kl_lf_loss_t_$(method)_Tp$(T_pred)_Tt$(T_train).npy")
        Lkl_t = npzread("./learned_generalization/kl_t_loss_t_$(method)_Tp$(T_pred)_Tt$(T_train).npy")
        Lf_norm = npzread("./learned_generalization/lf_norm_loss_t_$(method)_Tp$(T_pred)_Tt$(T_train).npy")
        return Lf[1:n_loss], Lkl_lf[1:n_loss], Lkl_t[1:n_loss], Lf_norm[1:n_loss]
end
...
function obtain_all_losses(all_methods, T_pred, T_train)
    ...
    for m_ in all_methods
        Lf[:, ii], Lkl_lf[:, ii], Lkl_t[:, ii], Lf_norm[:, ii] = load_losses(m_, T_pred, T_train)
		if m_ == "rot_inv_theta_liv_Pi"
			Lf_norm_ = npzread("./learned_generalization_t50/lf_norm_loss_t_$(m_)_Tp500_Tt50_over_t.npy")
