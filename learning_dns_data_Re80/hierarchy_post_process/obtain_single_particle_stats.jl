...
function load_data_mt(method, T_pred, T_train, mt)
	...
	tra_path = "./learned_data_mt_relation/traj_Tt$(T_train)_Tp$(T_pred)_h$(h)_$(IC)_$(method)_$(l_method)_Mt$(mt).npy"
	vel_path = "./learned_data_mt_relation/vels_Tt$(T_train)_Tp$(T_pred)_h$(h)_$(IC)_$(method)_$(l_method)_Mt$(mt).npy"
	rho_path = "./learned_data_mt_relation/rhos_Tt$(T_train)_Tp$(T_pred)_h$(h)_$(IC)_$(method)_$(l_method)_Mt$(mt).npy"
	accl = npzread(acc_path);
	traj = npzread(tra_path);
	vels = npzread(vel_path);
	rhos = npzread(rho_path);
...
# for Mt in mts
	...
	save_output_data(Vel_inc_pred, "./learned_generalization_mt_rel/vel_inc_pred_$(method)_t$(t_inc)_tp$(T_pred)_$(l_method)_Mt$(Mt).npy")
	save_output_data(Diff_pred, "./learned_generalization_mt_rel/diff_pred_$(method)_t$(t_inc)_tp$(T_pred)_$(l_method)_Mt$(Mt).npy")
