...
println(p_fin)
if Mt == 0.04
	p_fin[end] = 1/8 * p_fin[end];
end

if Mt == 0.16
	p_fin[end] = 8*p_fin[end];
end
println(p_fin)
...
function comparing_Ga(T_frame, width=0.22)
    gr(size=(700,600))
    x_s = -width
    x_e = width
	Ga_m(x) = kde(x, vec(accl[T_frame, :, :]));
...
function gen_data_files(accl, traj, vels, rhos)
	acc_path = "./learned_data/accl_Tt$(T_train)_Tp$(T_pred)_h$(h)_$(IC)_$(method)_$(l_method)_Mt$(Mt).npy"
	pos_path = "./learned_data/traj_Tt$(T_train)_Tp$(T_pred)_h$(h)_$(IC)_$(method)_$(l_method)_Mt$(Mt).npy"
	vel_path = "./learned_data/vels_Tt$(T_train)_Tp$(T_pred)_h$(h)_$(IC)_$(method)_$(l_method)_Mt$(Mt).npy"
	rho_path = "./learned_data/rhos_Tt$(T_train)_Tp$(T_pred)_h$(h)_$(IC)_$(method)_$(l_method)_Mt$(Mt).npy"
