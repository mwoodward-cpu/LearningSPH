...
"""
					Senstivity analysis of phys_inf_Wab

   learning c, α, β, γ, a, b
...
function fij(n, n2, rho, XX, VV, Vn, meanV, tke, r2, p, h, m, i)
  c_in, α_in, β_in, g_in, a_in, b_in, θ_in = p
  Pi = Pres(rho[n], c_in, g_in, po)
  Pj = Pres(rho[n2], c_in, g_in, po)
  Π = compute_Π(XX, VV, rho[n], rho[n2], α_in, β_in, h, c_in)
  summand = - m * (Pi/(rho[n]^2) + Pj/(rho[n2]^2) + Π) * XX[i] * H(sqrt(r2), h, a_in, b_in)
  f_ext = (θ_in / tke) * (Vn[i] .- meanV[i])
...
function obtain_sph_diff_A_model_deriv(X, V, p_in)
  c_in, α_in, β_in, g_in, a_in, b_in, θ_in = p_in
  ...
  for n in 1 : N
    mPdrho2[n] = m * P_d_rho2(rho[n], Pres, c_in, g_in, po);
  end
        ...
        for za_hash in x_hash[3] - 2 : x_hash[3] + 2
          ...
          for n2 in hash[xb_hash, yb_hash, zb_hash]
            ...
            if (close)
              ...
              dP_rhx = (∂P_rhoi2_∂x(rho[n], drhoi_dxi[n, 1], c_in, g_in, po) + ∂P_rhoi2_∂x(rho[n2], drhoj_dxi[n2, 1], c_in, g_in, po))
              dP_rhy = (∂P_rhoi2_∂y(rho[n], drhoi_dxi[n, 2], c_in, g_in, po) + ∂P_rhoi2_∂y(rho[n2], drhoj_dxi[n2, 2], c_in, g_in, po))
              dP_rhz = (∂P_rhoi2_∂z(rho[n], drhoi_dxi[n, 3], c_in, g_in, po) + ∂P_rhoi2_∂z(rho[n2], drhoj_dxi[n2, 3], c_in, g_in, po))
              ...
              ∂Π_∂zi = ∂Π_z(XX, VV, c_in, h, α_in, β_in, rho[n], rho[n2], drhoi_dxi[n, 3], drhoj_dxi[n, 3])
              ∂Π_∂ui = ∂Π_u(XX, VV, c_in, h, α_in, β_in, rho[n], rho[n2])
              ∂Π_∂vi = ∂Π_v(XX, VV, c_in, h, α_in, β_in, rho[n], rho[n2])
              ∂Π_∂wi = ∂Π_w(XX, VV, c_in, h, α_in, β_in, rho[n], rho[n2])
