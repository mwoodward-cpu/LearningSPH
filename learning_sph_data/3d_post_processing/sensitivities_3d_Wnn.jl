...
Learning W from SPH data by using a NN within an SPH simulator
...
W_nn(r, p) = sigma * re(p)([r/h])[1]

function H_nn(r, p)
  ∂W_∂r = ForwardDiff.derivative(r -> W_nn(r, p), r)
  # if r == 0.0
  #   out = sigma * (-3)/(h^2)
  # else out = ∂W_∂r / r
  # end
...
function ∂Hnn_∂r_r(r, p)
  ∂H_∂r = ForwardDiff.derivative(r -> H_nn(r, p), r)
  if r == 0.0
    out = 1e8;
  else out = ∂H_∂r / r
  end
...
function fij(n, n2, rho, XX, VV, r2, p, m, i)
  c = c_gt;
  Pi = Pres(rho[n], c, g)
  Pj = Pres(rho[n2], c, g)
  Π = compute_Π(XX, VV, rho[n], rho[n2], α, β, h, c)
  summand = - m * (Pi/(rho[n]^2) + Pj/(rho[n2]^2) + Π) * XX[i] * H_nn(sqrt(r2), p)
  return summand
end


function ∂fij_p(n, n2, rho, XX, VV, r2, p, m, i)
        return ForwardDiff.gradient(p -> fij(n, n2, rho, XX, VV, r2, p, m, i), p)
end
...
# function obtain_sph_diff_A_model_deriv(X, V, p_in, h)
...
function obtain_sph_diff_A_model_deriv(X, V, p_in)
      ...
      for ya_hash in x_hash[2] - 2 : x_hash[2] + 2
        ...
        for za_hash in x_hash[3] - 2 : x_hash[3] + 2
          ...
          for n2 in hash[xb_hash, yb_hash, zb_hash]
            ...
            if (close)
              tmp = m * W_nn(sqrt(r2), p_in); rho[n] += tmp;
              for i in 1 : D
                drhoi_dxi[n, i] += m * H_nn(sqrt(r2), p_in) * XX[i] #computes ∂ρᵢ/∂xᵢ
                drhoj_dxi[n, i] = m * H_nn(sqrt(r2), p_in) * XX[i] #computes ∂ρⱼ/∂xᵢ
              end
            end
          end
        end
  ...
  for n in 1 : N
    mPdrho2[n] = m * P_d_rho2(rho[n], Pres, c, g);
  end
      ...
      for ya_hash in x_hash[2] - 2 : x_hash[2] + 2
        ...
        for za_hash in x_hash[3] - 2 : x_hash[3] + 2
          ...
          for n2 in hash[xb_hash, yb_hash, zb_hash]
            ...
            if (close)
              Π = compute_Π(XX, VV, rho[n], rho[n2], α, β, h, c)
              tmp = -(mPdrho2[n] + mPdrho2[n2] + m*Π) * H_nn(sqrt(r2), p_in);
              for i in 1 : D
              ...
              ∂F_p[n, 4, :] .+= ∂fij_p(n, n2, rho, XX, VV, r2, p_in, m, 1)
              ∂F_p[n, 5, :] .+= ∂fij_p(n, n2, rho, XX, VV, r2, p_in, m, 2)
              ∂F_p[n, 6, :] .+= ∂fij_p(n, n2, rho, XX, VV, r2, p_in, m, 3)
              ...
              tmpH = H_nn(sqrt(r2), p_in);
              tmpW = W_nn(sqrt(r2), p_in);
              # tmp∂rH_r = ∂Hnn_∂r_r(sqrt(r2), h, p_in);
              ...
              dP_rhx = (∂P_rhoi2_∂x(rho[n], drhoi_dxi[n, 1], c, g) + ∂P_rhoi2_∂x(rho[n2], drhoj_dxi[n2, 1], c, g))
              dP_rhy = (∂P_rhoi2_∂y(rho[n], drhoi_dxi[n, 2], c, g) + ∂P_rhoi2_∂y(rho[n2], drhoj_dxi[n2, 2], c, g))
              dP_rhz = (∂P_rhoi2_∂z(rho[n], drhoi_dxi[n, 3], c, g) + ∂P_rhoi2_∂z(rho[n2], drhoj_dxi[n2, 3], c, g))
